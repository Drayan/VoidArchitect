...
namespace VoidArchitect::Platform
{
    VkDescriptorType TranslateEngineResourceTypeToVulkan(const Renderer::ResourceBindingType type)
    {
        ...
        switch (type)
        {
            ...
            default: VA_ENGINE_WARN("[VulkanPipeline] Unknown resource type, defaulting to none.");
    ...
    VkShaderStageFlagBits TranslateEngineShaderStageToVulkan(const Resources::ShaderStage stage)
    {
        ...
        switch (stage)
        {
            ...
            default: VA_ENGINE_WARN("[VulkanPipeline] Unknown shader stage, defaulting to all.");
                return VK_SHADER_STAGE_ALL_GRAPHICS;
        }
    }
    ...
    VkFormat TranslateEngineTextureFormatToVulkan(const Renderer::TextureFormat format)
    {
        switch (format)
        {
            ...
            default: VA_ENGINE_WARN(
                    "[VulkanUtils] Unknown texture format, defaulting to RGBA8_UNORM.");
                return VK_FORMAT_R8G8B8A8_UNORM;
        }
    }
    ...
    VkAttachmentLoadOp TranslateEngineLoadOpToVulkan(const Renderer::LoadOp op)
    {
        switch (op)
        {
            ...
            default: VA_ENGINE_WARN("[VulkanUtils] Unknown load op, defaulting to CLEAR.");
                return VK_ATTACHMENT_LOAD_OP_CLEAR;
        }
    }
    ...
    VkAttachmentStoreOp TranslateEngineStoreOpToVulkan(const Renderer::StoreOp op)
    {
        switch (op)
        {
            ...
            default: VA_ENGINE_WARN("[VulkanUtils] Unknown store op, defaulting to STORE.");
                return VK_ATTACHMENT_STORE_OP_STORE;
        }
    }

    VkFilter TranslateEngineTextureFilterToVulkan(const Resources::TextureFilterMode mode)
    {
        switch (mode)
        {
            case Resources::TextureFilterMode::Nearest:
                return VK_FILTER_NEAREST;
            case Resources::TextureFilterMode::Linear:
                return VK_FILTER_LINEAR;
            default: VA_ENGINE_WARN("[VulkanUtils] Unknown texture filter, defaulting to LINEAR.");
                return VK_FILTER_LINEAR;
        }
    }

    VkSamplerAddressMode TranslateEngineTextureRepeatToVulkan(
        const Resources::TextureRepeatMode mode)
    {
        switch (mode)
        {
            case Resources::TextureRepeatMode::Repeat:
                return VK_SAMPLER_ADDRESS_MODE_REPEAT;
            case Resources::TextureRepeatMode::MirroredRepeat:
                return VK_SAMPLER_ADDRESS_MODE_MIRRORED_REPEAT;
            case Resources::TextureRepeatMode::ClampToEdge:
                return VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE;
            case Resources::TextureRepeatMode::ClampToBorder:
                return VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_BORDER;
            default: VA_ENGINE_WARN("[VulkanUtils] Unknown texture repeat, defaulting to REPEAT.");
                return VK_SAMPLER_ADDRESS_MODE_REPEAT;
        }
    }
