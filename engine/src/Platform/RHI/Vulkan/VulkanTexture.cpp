...
namespace VoidArchitect::Platform
{
    ...
    void VulkanTexture2D::UpdateInternalData(IRenderingHardware& rhi, std::vector<uint8_t>& data)
    {
        auto& vulkanRhi = dynamic_cast<VulkanRHI&>(rhi);
        const auto& device = vulkanRhi.GetDeviceRef();
        ...
        // Create the staging buffer and image.
        auto staging = VulkanStagingBuffer(vulkanRhi, device, m_Allocator, data);
        m_Image = VulkanImage(
            vulkanRhi,
            device,
            m_Allocator,
