...
{
    ShaderDataDefinition::ShaderDataDefinition(const ShaderConfig& config, VAArray<uint8_t> code)
    ...
    {
        if (stage == "vertex") return Resources::ShaderStage::Vertex;
        if (stage == "pixel" || stage == "fragment") return Resources::ShaderStage::Pixel;
        if (stage == "compute") return Resources::ShaderStage::Compute;
        if (stage == "geometry") return Resources::ShaderStage::Geometry;
