...
namespace VoidArchitect
{
    ...
    Resources::RenderPassHandle RenderPassSystem::GetHandleFor(
        ...
        Renderer::PassPosition position)
    {
        ...
        if (!renderPassPtr)
        {
            VA_ENGINE_ERROR(
                ...
                config.name);
            return Resources::InvalidRenderPassHandle;
        }
    ...
    const Renderer::RenderPassConfig& RenderPassSystem::GetConfigFor(
        Resources::RenderPassHandle handle) const
    {
        VA_ENGINE_ASSERT(
            handle != Resources::InvalidRenderPassHandle,
            "Invalid render pass handle.");
    ...
    Resources::IRenderPass* RenderPassSystem::GetPointerFor(
        const Resources::RenderPassHandle handle) const
    {
        return m_RenderPasses[handle].get();
    }

    const Resources::RenderPassSignature& RenderPassSystem::GetSignatureFor(
        const Resources::RenderPassHandle passHandle) const
    {
        return m_RenderPasses[passHandle]->GetSignature();
    }

    Resources::RenderPassHandle RenderPassSystem::GetFreeHandle()
    {
    ...
    void RenderPassSystem::ReleasePass(const Resources::RenderPassHandle handle)
    {
