...
namespace VoidArchitect
{
    namespace Renderer
    {
        RenderSystem::RenderSystem(
            std::unique_ptr<Platform::IRenderingHardware>& rhi,
            std::unique_ptr<Window>& window)
            : m_ApiType(Platform::RHI_API_TYPE::Vulkan),
              m_RHI(std::move(rhi)),
            ...
            : m_MainCamera(45.0f, 1.f, 0.1f, 1000.f)
        {
            ...
            switch (apiType)
            {
                case Platform::RHI_API_TYPE::Vulkan:
                    m_RHI = nullptr; //std::make_unique<Platform::VulkanRHI>(window);
                    break;
                default:
                    break;
            }
        ...
        void RenderSystem::RenderFrame(const float frameTime)
        {
            ...
            for (const auto& step : executionPlan)
            {
                ...
                const Resources::RenderPassHandle passHandle = g_RenderPassSystem->GetHandleFor(
                    step.passConfig,
                    step.passPosition);
