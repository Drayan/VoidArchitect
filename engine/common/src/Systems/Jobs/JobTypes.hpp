...
namespace VoidArchitect::Jobs
{
    ...
    /// @brief Handle for referencing SyncPoint with generation-based validation
    using SyncPointHandle = Handle<SyncPoint>;

    /// @brief Invalid SyncPoint handle constant for convenience
    constexpr SyncPointHandle InvalidSyncPointHandle = SyncPointHandle{};
    ...
    struct JobResult
    {
        ...
        explicit JobResult(const Status s, const char* msg = "")
            : status(s),
              errorMessage(msg)
        {
        }
        ...
        static JobResult Failed(const char* message)
        {
        ...
        static JobResult Cancelled(const char* message = "Job was cancelled")
        {
    ...
    inline const char* GetPriorityString(const JobPriority priority)
    {
        switch (priority)
        {
    ...
    inline const char* GetJobStateString(const JobState state)
    {
        switch (state)
        {
    ...
    struct Job
    {
        ...
        /// @brief SyncPoint to signal when this job completes
        SyncPointHandle signalOnCompletion{InvalidSyncPointHandle};
        ...
        /// @brief Construct a job with function, sync point, and metadata
        /// @param func Function to execute
        /// @param signalSP SyncPoint to signal on completion
        /// @param name Debug name for the job
        /// @param prio Job priority
        /// @param affinity Preferred worker ID (optional)
        Job(
        ...
        /// @brief Construct a job from void function with sync point
        /// @param func Void function to execute
        /// @param signalSP SyncPoint to signal on completion
        /// @param name Debug name for the job
        /// @param prio Job priority
