...
namespace VoidArchitect::Platform
{
    ...
    VulkanDevice::VulkanDevice(
        ...
        const RHI::SurfaceHandle& surfaceHandle,
        RHI::SurfaceCreationCallback surfaceCallback,
        const VAArray<const char*>& requiredExtensions,
        ...
        : m_Allocator(allocator),
          ...
          m_SurfaceCallback(std::move(surfaceCallback)),
          ...
          m_ComputeQueue{}
    {
        ...
        if (surfaceHandle.IsDeferred() && m_SurfaceCallback.finalize)
        {
            void* nativeSurface = nullptr;
            if (m_SurfaceCallback.finalize(
                m_Instance,
                surfaceHandle.GetCreationData(),
                &nativeSurface))
            {
        ...
        else if (surfaceHandle.IsReady())
        {
            m_Surface = surfaceHandle.GetAs<VkSurfaceKHR>();
            VA_ENGINE_INFO("[VulkanDevice] Surface created.");
        }
        else
        {
...
#endif
        if (m_Instance != VK_NULL_HANDLE) vkDestroyInstance(m_Instance, m_Allocator);
    ...
    void VulkanDevice::CreateInstance()
    {
        ...
        unsigned int extensionCount = m_RequiredExtensions.size();
        char const* const* extensions = m_RequiredExtensions.data();
        ...
        {
            std::stringstream ss;
            for (unsigned int i = 0; i < extensionCount; ++i) ss << extensions[i] << std::endl;
    ...
    void VulkanDevice::CreateLogicalDevice(const DeviceRequirements& requirements)
    {
        ...
        if (VA_VULKAN_CHECK_RESULT(
            vkCreateCommandPool( m_LogicalDevice, &poolCreateInfo, m_Allocator, &
    ...
    void VulkanDevice::FindQueueFamilies()
    {
        ...
        for (const auto& queueFamily : queueFamilies)
        {
            ...
            VA_VULKAN_CHECK_RESULT_WARN(
                vkGetPhysicalDeviceSurfaceSupportKHR( m_PhysicalDevice, queueFamilyIndex, m_Surface,
                    &presentSupport));

            if (presentSupport) m_PresentQueueFamilyIndex = queueFamilyIndex;

            if (m_GraphicsQueueFamilyIndex.has_value() && m_TransferQueueFamilyIndex.has_value() &&
                m_ComputeQueueFamilyIndex.has_value() && presentSupport) break;
    ...
    {
        for (unsigned int i = 0; i < extensionCount; i++) delete[] extensions[i];
    ...
    void VulkanDevice::CreateDebugMessenger()
    {
        ...
        const auto vkCreateDebugUtilsMessengerEXT = reinterpret_cast<
        ...
        VA_VULKAN_CHECK_RESULT_CRITICAL(
            vkCreateDebugUtilsMessengerEXT( m_Instance, &debugCreateInfo, m_Allocator, &
                m_DebugMessenger));
    ...
    void VulkanDevice::DestroyDebugMessenger() const
    {
        ...
        {
            const auto vkDestroyDebugUtilsMessengerEXT = reinterpret_cast<
