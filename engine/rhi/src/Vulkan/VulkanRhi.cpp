...
{
    std::unique_ptr<Platform::IRenderingHardware> CreateVulkanRHI(std::unique_ptr<Window>& window)
    {
        return std::make_unique<Platform::VulkanRHI>(window);
    }
}
...
namespace VoidArchitect::Platform
{
          ...
          m_Allocator(nullptr)
    {
        ...
        m_SurfaceFactory = m_Window->CreateSurfaceFactory();
        if (!m_SurfaceFactory)
        {
            VA_ENGINE_CRITICAL("[VulkanRHI] Failed to create surface factory.");
            throw std::runtime_error("Failed to create surface factory.");
        }

        m_SurfaceHandle = m_SurfaceFactory->CreateSurface(
            RHI_API_TYPE::Vulkan,
            RHI::SurfaceCreateInfo{});
        if (!m_SurfaceHandle.IsDeferred() && !m_SurfaceHandle.IsReady())
        {
            VA_ENGINE_CRITICAL("[VulkanRHI] Failed to create surface.");
            throw std::runtime_error("Failed to create surface.");
        }

        m_SurfaceCallback = m_SurfaceFactory->GetCreationCallback(RHI_API_TYPE::Vulkan);
        VA_ENGINE_INFO("[VulkanRHI] Surface factory and handle created.");
    ...
    VulkanRHI::~VulkanRHI()
    {
        ...
        if ((m_SurfaceHandle.IsReady() || m_SurfaceHandle.IsDeferred()) && m_SurfaceFactory)
        {
            m_SurfaceFactory->DestroySurface(m_SurfaceHandle);
        }
    ...
    void VulkanRHI::CreateDevice()
    {
        ...
        auto requiredExtensions = m_SurfaceFactory->GetRequiredVulkanExtensions();
    ...
    void VulkanRHI::BeginRenderPass(
        const Resources::RenderPassHandle passHandle,
    ...
    void VulkanRHI::BindRenderState(Resources::RenderStateHandle stateHandle)
    {
    ...
    void VulkanRHI::BindMaterial(
        ...
        const Resources::RenderStateHandle stateHandle)
    {
