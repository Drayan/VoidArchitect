...
namespace VoidArchitect::Resources
{
    ShaderStage ShaderStageFromString(const std::string& stage)
    {
        ...
        if (stage == "pixel" || stage == "fragment" || stage == "frag") return ShaderStage::Pixel;
        if (stage == "compute") return ShaderStage::Compute;
        if (stage == "geometry") return ShaderStage::Geometry;
        if (stage == "tessellation_control") return ShaderStage::TessellationControl;
        if (stage == "tessellation_evaluation") return ShaderStage::TessellationEvaluation;
        if (stage == "all") return ShaderStage::All;
