...
// pub(super) fn new(device: &VulkanDevice, width: u32, height: u32) -> Result<Self, String> {
//     Self::create_swapchain(device, width, height)
// }
...
//     height: u32,
// ) -> Result<(), String> {
//     Self::destroy_swapchain(device, swapchain_loader, self);
//     *self = match Self::create_swapchain(vulkan_device, width, height) {
//         Ok(swapchain) => swapchain,
...
//         Ok((index, _)) => Ok(index as usize),
//         Err(vk::Result::ERROR_OUT_OF_DATE_KHR) => {
//             self.recreate(vulkan_device, device, swapchain_loader, width, height)?;
//             Ok(usize::MAX)
//         }
//         Err(e) => Err(format!("Failed to acquire next image index: {:?}", e)),
//     }
