...
use super::session_manager::{RegisterSession, SessionManagerActor}; // Import RegisterSession message
use actix::{Actor, ActorContext, Addr, Context, Handler, Message}; // Added ActorContext
use tokio::net::TcpStream;
use uuid::Uuid; // Import RegisterSession message
...
impl HandshakeActor {
    ...
    fn handle_client_response(&mut self, ctx: &mut Context<Self>, _response: Vec<u8>) {
        ...
        if let Some(stream) = self.stream.take() {
            ...
            self.session_manager.do_send(RegisterSession {
                session_id,
                stream,
                client_uuid,
            });
...
mod tests {
    use super::*;
    use crate::old::session_manager::SessionManagerActor;
    use actix::Actor;
